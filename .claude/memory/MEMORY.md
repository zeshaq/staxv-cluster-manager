...
- [Physical servers (Redfish)](servers.md) — BMC enrollment, power actions, hardware inventory, thermal/power sensors, Virtual Media (ISO mount)
- [Network devices (Cisco SSH)](network.md) — IOS/IOS-XE switches + routers: enroll, probe, live health (Phase 1 shipped; VLANs + interface IPs planned)
- [ISO library](isos.md) — install-media catalog, upload + URL import, BMC-facing serve route
